...
const BODY_STYLES = ['Sedan', 'Coupe', 'Hatchback', 'SUV', 'Crossover', 'Truck', 'Minivan', 'Wagon'];

// ---------- defaults ----------
function blankVehicle() {
  return {
    id: uid(), archived: false, excludeReason: '', excludedAt: 0, createdAt: Date.now(), viewedAt: Date.now(),
    make: '', model: '', year: new Date().getFullYear(), trim: '', bodyStyle: 'Sedan',
    condition: 'New', mileage: 0, color: '', dealer: '', listingUrl: '', accent: '#b4552d',
    powertrain: 'gas', notes: '',
    specs: {},
    ratings: {},
...
function leaseCalc(v) {
  const p = v.pricing, l = v.lease;
  const cap = Math.max(0, (p.sellingPrice || 0) - (l.downPayment || 0) - (p.tradeValue || 0) - (p.incentives || 0));
...
function ownershipCalc(v) {
  const o = v.ownership;
  const perYear = energyCostPerYear(v) + (o.insuranceYr || 0) + (o.maintenanceYr || 0);
  return { perYear, y1: perYear, y3: perYear * 3, y5: perYear * 5, energy: energyCostPerYear(v) };
}
function avgRating(v) {
  const vals = RATING_CATS.map(c => v.ratings[c.key]).filter(x => typeof x === 'number' && x > 0);
  if (!vals.length) return 0;
  return vals.reduce((a, b) => a + b, 0) / vals.length;
}

// ---------- decision matrix ----------
// metric: fn(v) -> raw value; dir: 'high' good or 'low' good
...
function matrixScores(vehicles, factors) {
  ...
  factors.forEach(f => {
    ...
    norm[f.metric] = active.map(val => {
      const raw = m.fn(active[0]); return null; // placeholder, computed below
    });
    f._min = min; f._max = max;
  });
  return active.map(v => {
    ...
    factors.forEach(f => {
      ...
      if (!m) return;
      const raw = m.fn(v);
      const span = (f._max - f._min) || 1;
      let n = (raw - f._min) / span; // 0..1, high raw -> 1
      if (m.dir === 'low') n = 1 - n;
      const contrib = (n * 100) * ((f.weight || 0) / totalW);
      breakdown[f.metric] = { raw, norm: n, contrib };
      score += contrib;
    });
...
function loadStore() {
  ...
  return { vehicles: SEED_VEHICLES(), matrix: DEFAULT_MATRIX, compareIds: null };
}
...
function useStore() {
  ...
  const api = useMemo(() => ({
    ...
    matrix: state.matrix || DEFAULT_MATRIX,
    addVehicle(v) {
      const nv = { ...blankVehicle(), ...v, id: uid(), createdAt: Date.now(), viewedAt: Date.now() };
      setState(s => ({ ...s, vehicles: [nv, ...s.vehicles] }));
      return nv.id;
    },
    updateVehicle(id, patch) {
      setState(s => ({ ...s, vehicles: s.vehicles.map(v => v.id === id ? deepMerge(v, patch) : v) }));
    },
    replaceVehicle(id, full) {
      setState(s => ({ ...s, vehicles: s.vehicles.map(v => v.id === id ? { ...full, id } : v) }));
    },
    removeVehicle(id) {
      setState(s => ({ ...s, vehicles: s.vehicles.filter(v => v.id !== id) }));
    },
    toggleArchive(id) {
      setState(s => ({ ...s, vehicles: s.vehicles.map(v => v.id === id ? { ...v, archived: !v.archived } : v) }));
    },
    setExcluded(id, excluded, reason = '') {
      setState(s => ({ ...s, vehicles: s.vehicles.map(v => v.id === id
        ? { ...v, archived: excluded, excludeReason: excluded ? (reason || v.excludeReason || '') : '', excludedAt: excluded ? Date.now() : 0 }
        : v) }));
    },
    duplicateVehicle(id) {
      let newId = null;
      setState(s => {
        ...
        copy.id = uid(); newId = copy.id; copy.trim = (copy.trim || '') + ' (scenario)'; copy.createdAt = Date.now();
        const idx = s.vehicles.findIndex(x => x.id === id);
        const arr = [...s.vehicles]; arr.splice(idx + 1, 0, copy);
        return { ...s, vehicles: arr };
      });
      return newId;
    },
    touchViewed(id) {
      setState(s => ({ ...s, vehicles: s.vehicles.map(v => v.id === id ? { ...v, viewedAt: Date.now() } : v) }));
    },
    setMatrix(matrix) { setState(s => ({ ...s, matrix })); },
    resetAll() { setState({ vehicles: SEED_VEHICLES(), matrix: DEFAULT_MATRIX }); },
  }), [state]);
...
// ---------- seed data (realistic mix: gas / hybrid / ev) ----------
function SEED_VEHICLES() {
  const base = blankVehicle;
  const mk = (data) => ({ ...base(), ...data, id: uid() });
  return [
    mk({
...
Object.assign(window, {
  uid, SPEC_FIELDS, RATING_CATS, TESTDRIVE_CATS, POWERTRAINS, BODY_STYLES,
  blankVehicle, taxesOn, outTheDoor, financeCalc, leaseCalc, ownershipCalc, energyCostPerYear,
  avgRating, MATRIX_METRICS, DEFAULT_MATRIX, matrixScores, useStore, SEED_VEHICLES, deepMerge,
});
